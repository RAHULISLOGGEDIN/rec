...
function AnotherComponent() {
  ...
  useEffect(() => {
    console.log("Another component got mounted");
    return () => {
      console.log("Another component is about to be mounted");
    };
  ...
  return (
    <div>
      I am present on the UI. the state itself <span>{name}</span>
      the state itself <span>{count}</span>
      <button onClick={() => setName(name + "a")}>Increase name</button>
